...
### 6.2 为什么 \(\rho\) 用 \(\mathrm{AI}\) 而不是 \(S_Z\)

一口井时 \(\mathrm{AI}\to\mathrm{UCS}\) 的 GP 会把该孔拟合到 \(\rho_{S_Z}\approx 1\)，若把这个 \(\rho\) 送进 Doyen 更新，远处就退化成 \(S_Z\)。改用物理次变量 \(\rho=\mathrm{corr}(\mathrm{UCS},\mathrm{AI})\)：即使 GP 在训练孔上几乎完美，更新幅度仍受井–阻抗真实相关限制（本基准约 0.76）。次变量场本身仍用标定后的 \(S_Z\)（与 UCS 同单位）。

Xu/Journel 的外漂移克里金（漂移 = 波阻抗）仍可通过 `geostats.regression_kriging_3d` 使用，但 \(S_F\) 统一走 Doyen 更新，避免再维持一条单井分支。
...
不确定度图把“开关”画出来：一口井时 MWD 的 \(\sigma_M\) 几乎全场都大，Doyen 更新的 \(\sigma_F\) 被 \((1-\rho^2)\) 压住；12 口井时只在靶心附近把方差收到硬数据。
...
3. 每个 \(n\) 重新做：PG-GPR → 回归克里金 → 用这 \(n\) 口孔标定 \(g(\mathrm{AI})\) → Doyen 共定位更新；
