...
本文给出本仓库三维岩石强度场 \(S(x,y,z)\) 的完整计算方案：从随钻参数与叠后地震出发，经可推导的正反演与克里金，再到外漂移克里金融合。插值核、变差函数与文献推导的专文见 [`插值方法.md`](插值方法.md)。所有符号与实现一一对应（见文末模块对照）。定量结论来自固定种子的共定位合成矿山基准（`seed=42`，14 口钻孔，网格 \(25\times 40\times 40\)，块段 \(50\times 80\times 40\,\mathrm{m}\)，默认工作面 \(20\times 50\,\mathrm{m}\)），图件在 [`images/`](images/) 中跟踪保存。
...
仅用钻孔把 UCS **插值**到三维，即使缩到 \(20\times 50\,\mathrm{m}\) 的工作面、14 口孔按梅花均匀布置在内部，相邻孔距仍约一个工作面尺度。克里金在孔旁忠实，孔间变光滑：硬矿体轮廓发糊，断裂和蚀变晕对不上。
...
3. 用**外漂移克里金**把 \(S_Z\) 当作漂移、孔点 UCS 当作硬数据。残差变程取孔距，硬矿体峰值不被 \(\rho\) 打折；单口井才走 Doyen 更新。
...
- 14 口孔按爆破常用的**梅花 / 三角网格**近似均匀铺在默认 \(20\times 50\,\mathrm{m}\) 工作面**内部**（邻行错半个孔距，中列补两个孔，格子相对工作面内缩约三分之一孔距，不贴边）。全块仍是 \(50\times 80\,\mathrm{m}\)，便于把工作面平移到别处。1→N 实验的嵌套前缀按最远点排序，稀疏子集同样铺开，而不是挤在一角。孔内每个深度节点都有样本。

![梅花布孔](images/hole_layout.png)

默认模型：\(X\in[0,50]\,\mathrm{m}\)，\(Y\in[0,80]\,\mathrm{m}\)，标高 \(0\to -40\,\mathrm{m}\)，约 10 个台阶。下文所有图与表均来自该模型、`seed=42`、14 口孔。
...
- 14 口孔按梅花铺在默认 \(20\times 50\,\mathrm{m}\) 工作面内部（离开工作面边界约三分之一孔距，中列补两个孔）；全块仍为 \(50\times 80\,\mathrm{m}\) 以便平移。胜任度场仍含有矿体/软弱带尺度的横向结构；
- 无横向样品时，残差克里金的最优解就是“回到均值”，于是一口井时 \(S_M\) 几乎是平面常数；孔变密后能勾出硬矿体轮廓，但孔间仍然过平滑；
- \(\sigma_M\) 在孔旁小、离孔大——这个方差后面会被融合用掉，而不是被丢掉。

下图同一色标：14 口梅花孔的克里金只在靶心附近有值，孔间几乎是深度趋势，硬矿体的亮斑基本丢了。
...
井（准、稀）+ 地震（连续、不准）是储层建模的经典设定。Xu、Tran、Srivastava、Journel（SPE 24742）给出两种做法：**外漂移克里金**（KED：井为硬数据，地震为漂移）和**共定位协克里金**。Doyen、den Boer、Pillet（SPE 36498）把共定位协克里金写成对井克里金的贝叶斯更新。

Doyen 更新在远处按 \(\rho\) 把次变量异常往均值缩。本基准 \(\rho\approx 0.76\)，硬矿体在 \(S_Z\) 上的峰值会被压到约四分之三，平面图上矿体发糊。KED 不这么做：漂移系数乘在整张 \(S_Z\) 上（本基准约 \(0.84\,S_Z-0.30\,z\)），残差再按**孔距**插值，峰值保住，孔间力学残差也不会被铺到全块。

把已经插好的 \(S_M\) 和 \(S_Z\) 当成**独立**观测做逆方差混合，假设不成立——两路共用同一批标定井。\(S_F\) 按 KED 构造，孔轨迹体素再写回 PG-GPR 点估计。

### 6.1 两口井及以上：外漂移克里金

主变量为孔点 UCS \(\hat\mu_{\mathrm{hole}}\)，漂移为深度与标定强度

\[
m_{\mathrm{KED}}(\mathbf{u})=\alpha_0+\alpha_1 z+\alpha_2 S_Z(\mathbf{u}),
\]

\(\boldsymbol{\alpha}\) 由孔点最小二乘估计。残差 \(r_i=\hat\mu_{\mathrm{hole}}(\mathbf{u}_i)-m_{\mathrm{KED}}(\mathbf{u}_i)\) 做三维普通克里金，球状模型的变程取 \(1.15\times\) 中位孔距（本基准约 \(16.5\,\mathrm{m}\)），垂向各向异性系数 \(3\)（沿孔采样密、横向稀）。实现：`geostats.regression_kriging_3d(..., residual_range=..., anisotropy_scaling_z=3)`。

- 孔上钉住 \(\hat\mu_{\mathrm{hole}}\)（硬数据）；
- 孔旁 \(r(\mathbf{u})\) 带上蚀变/风化等阻抗看不见的力学残差；
- 离孔后残差回 0，\(S_F\to m_{\mathrm{KED}}\)，结构与峰值来自 \(S_Z\)，不再被 \(\rho\) 打折。

### 6.2 单口井：Doyen 更新

一口井时 \(\mathrm{AI}\to\mathrm{UCS}\) 的 GP 会把该孔拟合到 \(\rho_{S_Z}\approx 1\)，KED 的 \(S_Z\) 漂移会把过拟合铺到全块。此时改用 Doyen 贝叶斯更新（`fusion.uncertainty_fusion.doyen_collocated_update`），\(\rho=\mathrm{corr}(\mathrm{UCS},\mathrm{AI})\) 限制更新幅度。
...
- 横向结构与硬矿体峰值来自 \(S_Z\)（连续波阻抗标定），KED 不按 \(\rho\) 缩峰；
- 孔旁力学细节来自残差克里金（变程 = 孔距），是硬数据而不是权重折中；
- 风化等随深度的部分进漂移里的 \(z\) 项，全块都有，不必等孔间插值。

下图上一行同一 UCS 色标：强度真值、钻孔插值、波阻抗插值、融合插值。下一行是预测减去真值的残差热力图（红偏高、蓝偏低，三列共用色标）：钻孔残差在硬矿体处最深；波阻抗残差轮廓对了但蚀变带估计偏高；融合残差最浅，矿体峰值不再被压矮。

![融合优势对比（含预测−真值）](images/fusion_advantage.png)

### 6.5 平面图上看着像全是波阻抗——钻孔是不是没用了？

不是。梅花均匀布孔之后，残差克里金只在靶心一个孔距内有值，孔间回落到漂移 \(m_{\mathrm{KED}}\)：结构仍来自波阻抗，幅值和孔旁力学残差被钻孔拉开。钻孔另外还干一件阻抗自己做不了的事：

1. **把波阻抗翻译成强度。** \(S_Z=g(\mathrm{AI})\) 的 GP 只能在共定位孔点上训练。没有岩心/MWD 的 UCS，地震再连续也只是阻抗，不是爆破用的强度。
2. **在孔轨迹上钉住力学真值，并在一个孔距内插值力学残差。** 蚀变、风化会改 UCS，却几乎不改 AI。沿孔剖面里，橙色“仅波阻抗”会偏离真值，绿色融合与 MWD 重合。14 口孔轨迹上：仅波阻抗 RMSE \(=7.23\,\mathrm{MPa}\)，融合 \(=2.41\,\mathrm{MPa}\)（与仅钻孔相同）。

![沿孔剖面：融合钉回真值](images/along_hole_profiles.png)

权重图把分工画出来：\(w_{\mathrm{MWD}}\) 在梅花靶心为 1（残差克里金方差低），孔间回落。

![MWD 融合权重](images/mwd_fusion_weight.png)

所以：钻孔**不是用来把全矿插值填满的**（那件事波阻抗更擅长），而是**标定器 + 硬数据 + 孔距尺度的力学残差**。梅花内缩布置让这些硬数据均匀盖住 \(20\times 50\,\mathrm{m}\) 工作面内部。合成模型的细尺度胜任度比孔距还短，孔间不可能还原到真值；融合相对两路单独仍把工作面体积 RMSE 从 8.4 / 8.0 降到 5.2。

不确定度图：一口井时走 Doyen，\(\sigma_F\) 被 \((1-\rho^2)\) 压住；14 口井时 KED 残差方差只在靶心附近低。

![MWD 与融合不确定度：1 口 vs 14 口](images/uncertainty_1_vs_all.png)
...
1. 生成 14 口孔的矿山，**只反演一次**波阻抗（地震覆盖与孔数无关）；
2. 取钻孔集合的嵌套前缀 \(\{1\}\subset\{1,2\}\subset\cdots\subset\{1,\ldots,14\}\)（`datasets.subset_holes`）；
3. 每个 \(n\) 重新做：PG-GPR → 回归克里金 → 用这 \(n\) 口孔标定 \(g(\mathrm{AI})\) → KED 融合（\(n=1\) 走 Doyen）；
4. 与同一份 \(\mathrm{UCS}_{\mathrm{true}}\) 在 \(20\times 50\,\mathrm{m}\) 工作面上比 \(R^2\)、RMSE；并单独统计距最近孔超过 \(0.4\times\) 中位孔距（本基准约 \(5\,\mathrm{m}\)）的远场 RMSE，以及孔轨迹上的 RMSE。

### 7.1 定性：插值随孔数变密，融合在标定可用后给出结构

![仅钻孔克里金，n = 1…14](images/well_series_mwd.png)

仅钻孔：\(n=1\) 几乎是平面常数；孔按最远点顺序增加后，梅花格子逐渐出现。到 \(n=14\) 仍比真值光滑，硬矿体峰值明显偏弱。

![融合强度场，n = 1…14](images/well_series_fused.png)

融合：\(n=2\) 起空间格局已接近 \(n=14\)（硬矿体、软弱带）。再增加钻孔主要是孔旁修订和标定微调。

![1 / 3 / 6 / 14 口钻孔对比](images/well_series_compare.png)
...
1. **一口井时融合不会盲目把过拟合的 GP 铺满工作面。** \(n=1\) 仅波阻抗 RMSE 9.13，纯插值 11.57；Doyen 用 \(\rho(\mathrm{UCS},\mathrm{AI})\) 限制更新，融合 8.58，优于两路单独。
2. **两口井起 KED，体积上融合优于两路单独。** \(n=14\) 时工作面 RMSE 5.17，低于仅波阻抗 8.02 和仅钻孔 8.37。
3. **孔上融合 = MWD，远好于仅波阻抗。** 14 口孔轨迹上 2.41 vs 7.23；孔体素写回点估计。
4. **加孔主要改善插值和标定，结构仍来自阻抗。** 仅钻孔 \(R^2\) 从 0.24 升到 0.60；融合场的硬矿体轮廓在 \(n=2\)–\(4\) 已经出现，且峰值不再被 \(\rho\) 压矮。中列多两个孔主要补上工作面中部的空白。
...
同一 \(20\times 50\,\mathrm{m}\) 工作面真值体积上：
...
补充：波阻抗反演本身 \(R^2(\mathrm{AI})=0.844\)。孔轨迹上融合 RMSE \(=2.41\)（与仅钻孔相同），仅波阻抗 \(=7.23\)。

- 相对**仅钻孔**，融合把工作面体积 RMSE 从 8.37 降到 5.17，平面图上硬矿体从“看不见”变成有峰值；
- 相对**仅波阻抗**，体积 RMSE 8.02 → 5.17，蚀变带不再整片偏高，孔上真值不被冲掉；
- 相对**简单平均**，体积 RMSE 7.14 → 5.17，切片残差 4.8 vs 仅阻抗 5.8 vs 仅钻孔 11.9。
...
| 外漂移克里金 | \(S_F=\alpha_0+\alpha_1 z+\alpha_2 S_Z+\mathrm{OK}(r)\)，变程 = 孔距 | `geostats.regression_kriging_3d` |
| 单井 Doyen 更新 | \(y_{\mathrm{cc}}'=y_k'+\lambda(y_s'-\rho y_k')\) | `fusion.uncertainty_fusion.doyen_collocated_update` |
