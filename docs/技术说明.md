...
在本基准上简单平均 \(R^2=0.716\)，精度加权（加钻孔锚定）\(0.717\)，体积几乎打平（RMSE 7.55 vs 7.54，锚定后 MAE 5.25 vs 5.39）。梅花内缩布孔之后 \(S_M\) 在块段内部不再大面积空白，固定 \(w=1/2\) 不再明显砸结构。本方案的价值是空间变权（孔上钉住钻孔真值、孔间信阻抗）以及 \(\sigma_F\)、\(w_{\mathrm{MWD}}\)。
...
\qquad
S_{\mathrm{prec}}=w_M S_M+(1-w_M)S_Z,
\qquad
S_F=w_{\mathrm{hole}}S_M+(1-w_{\mathrm{hole}})S_{\mathrm{prec}}.
\]

\(w_M\) 由克里金/标定方差给出；\(w_{\mathrm{hole}}\) 是孔口紧支撑核（约 \(0.4\times\) 中位孔距），在轨迹上为 1、孔间为 0。精度加权只改**插值区**；钻孔硬数据不会被波阻抗抵消。所以：
...
| ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| 1 | 11.82 | 0.303 | 12.77 | 12.18 | 0.261 | 0.01 | 0.23 |
| 2 | 11.79 | 0.308 | 9.32 | 8.75 | 0.619 | 8.20 | 0.07 |
| 4 | 11.53 | 0.338 | 8.98 | 8.91 | 0.605 | 8.23 | 1.58 |
| 6 | 10.93 | 0.405 | 8.94 | 8.42 | 0.647 | 7.63 | 2.81 |
| 8 | 10.64 | 0.436 | 8.93 | 7.98 | 0.683 | 7.76 | 2.73 |
| 12 | 9.49 | 0.552 | 9.01 | 7.54 | 0.717 | 7.94 | 2.56 |
...
2. **孔数够做标定时，体积上融合优于两路单独。** \(n=12\) 时 RMSE 7.54，低于仅波阻抗 9.01 和仅钻孔 9.49。
3. **孔上融合 = MWD，远好于仅波阻抗。** 12 口孔轨迹上 2.56 vs 7.94；融合不再把孔上真值冲掉。
...
| **不确定度加权融合（本方案）** | **0.717** | **7.54** | **5.25** |

补充：波阻抗反演本身 \(R^2(\mathrm{AI})=0.839\)。孔轨迹上融合 RMSE \(=2.56\)（与仅钻孔相同），仅波阻抗 \(=7.94\)。

- 相对**仅钻孔**，融合把体积 RMSE 从 9.49 降到 7.54，平面图上硬矿体从“看不见”变成有轮廓；
- 相对**仅波阻抗**，体积 RMSE 9.01 → 7.54，孔上真值不再被冲掉（见沿孔剖面与残差热力图）；
- 相对**简单平均**，体积几乎打平，并给出 \(\sigma_F\) 和 \(w_{\mathrm{MWD}}\)，孔旁钉住钻孔。
...
| 精度加权 | \(\mu_f=(\tau_M\mu_M+\tau_Z\mu_Z)/\tau_f\) | `fusion.uncertainty_fusion.precision_weighted_fusion` |
| 钻孔硬数据锚定 | \(S_F=w_{\mathrm{hole}}S_M+(1-w_{\mathrm{hole}})S_{\mathrm{prec}}\) | `fusion.uncertainty_fusion.borehole_anchor_weight` |
