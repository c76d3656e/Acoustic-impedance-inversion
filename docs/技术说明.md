...
本文给出本仓库三维岩石强度场 \(S(x,y,z)\) 的完整计算方案：从随钻参数与叠后地震出发，经可推导的正反演与克里金，再到外漂移克里金融合。插值核、变差函数与文献推导的专文见 [`插值方法.md`](插值方法.md)。所有符号与实现一一对应（见文末模块对照）。定量结论来自固定种子的共定位合成矿山基准（`seed=42`，12 口钻孔，网格 \(25\times 40\times 40\)，块段 \(50\times 80\times 40\,\mathrm{m}\)，默认工作面 \(20\times 50\,\mathrm{m}\)），图件在 [`images/`](images/) 中跟踪保存。
...
仅用钻孔把 UCS **插值**到三维，即使缩到 \(20\times 50\,\mathrm{m}\) 的工作面、12 口孔按梅花均匀布置在内部，相邻孔距仍约一个工作面尺度。克里金在孔旁忠实，孔间变光滑：硬矿体轮廓发糊，断裂和蚀变晕对不上。
...
- 12 口孔按爆破常用的**梅花 / 三角网格**近似均匀铺在默认 \(20\times 50\,\mathrm{m}\) 工作面**内部**（邻行错半个孔距，格子相对工作面内缩约三分之一孔距，不贴边）。全块仍是 \(50\times 80\,\mathrm{m}\)，便于把工作面平移到别处。1→N 实验的嵌套前缀按最远点排序，稀疏子集同样铺开，而不是挤在一角。孔内每个深度节点都有样本。
...
- 12 口孔按梅花铺在默认 \(20\times 50\,\mathrm{m}\) 工作面内部（离开工作面边界约三分之一孔距）；全块仍为 \(50\times 80\,\mathrm{m}\) 以便平移。胜任度场仍含有矿体/软弱带尺度的横向结构；
...
2. **在孔轨迹上钉住力学真值，并在一个孔距内插值力学残差。** 蚀变、风化会改 UCS，却几乎不改 AI。沿孔剖面里，橙色“仅波阻抗”会偏离真值，绿色融合与 MWD 重合。12 口孔轨迹上：仅波阻抗 RMSE \(=7.04\,\mathrm{MPa}\)，融合 \(=2.49\,\mathrm{MPa}\)（与仅钻孔相同）。
...
所以：钻孔**不是用来把全矿插值填满的**（那件事波阻抗更擅长），而是**标定器 + 硬数据 + 孔距尺度的力学残差**。梅花内缩布置让这些硬数据均匀盖住 \(20\times 50\,\mathrm{m}\) 工作面内部。合成模型的细尺度胜任度比孔距还短，孔间不可能还原到真值；融合相对两路单独仍把工作面体积 RMSE 从 8.7 / 8.1 降到 5.6。

不确定度图：一口井时走 Doyen，\(\sigma_F\) 被 \((1-\rho^2)\) 压住；12 口井时 KED 残差方差只在靶心附近低。
...
| ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| 1 | 11.55 | 0.243 | 9.13 | 8.56 | 0.584 | 0.00 | 0.84 |
| 2 | 11.31 | 0.274 | 9.93 | 8.70 | 0.571 | 5.99 | 1.45 |
| 4 | 10.57 | 0.366 | 8.06 | 6.09 | 0.790 | 8.74 | 2.66 |
| 6 | 9.83 | 0.452 | 8.29 | 6.04 | 0.793 | 7.97 | 2.45 |
| 8 | 9.24 | 0.515 | 8.04 | 5.72 | 0.814 | 7.81 | 2.45 |
| 12 | 8.65 | 0.575 | 8.11 | 5.60 | 0.822 | 7.04 | 2.49 |

（完整表见 [`images/well_count_metrics.csv`](images/well_count_metrics.csv)。远场阈值为 \(0.4\times\) 中位孔距。孔上融合 RMSE 与仅钻孔相同：硬数据被钉住。）

读表：

1. **一口井时融合不会盲目把过拟合的 GP 铺满工作面。** \(n=1\) 仅波阻抗 RMSE 9.13，纯插值 11.55；Doyen 用 \(\rho(\mathrm{UCS},\mathrm{AI})\) 限制更新，融合 8.56，优于两路单独。
2. **两口井起 KED，体积上融合优于两路单独。** \(n=12\) 时工作面 RMSE 5.60，低于仅波阻抗 8.11 和仅钻孔 8.65。
3. **孔上融合 = MWD，远好于仅波阻抗。** 12 口孔轨迹上 2.49 vs 7.04；孔体素写回点估计。
4. **加孔主要改善插值和标定，结构仍来自阻抗。** 仅钻孔 \(R^2\) 从 0.24 升到 0.58；融合场的硬矿体轮廓在 \(n=2\)–\(4\) 已经出现，且峰值不再被 \(\rho\) 压矮。
...
| --- | ---: | ---: | ---: |
| 仅 MWD 克里金插值 | 0.575 | 8.65 | 5.36 |
| 仅波阻抗标定 | 0.626 | 8.11 | 4.52 |
| 简单加权 \(w=1/2\) | 0.688 | 7.41 | 4.69 |
| **外漂移克里金（本方案）** | **0.822** | **5.60** | **3.45** |

补充：波阻抗反演本身 \(R^2(\mathrm{AI})=0.844\)。孔轨迹上融合 RMSE \(=2.49\)（与仅钻孔相同），仅波阻抗 \(=7.04\)。

- 相对**仅钻孔**，融合把工作面体积 RMSE 从 8.65 降到 5.60，平面图上硬矿体从“看不见”变成有峰值；
- 相对**仅波阻抗**，体积 RMSE 8.11 → 5.60，蚀变带不再整片偏高，孔上真值不被冲掉；
- 相对**简单平均**，体积 RMSE 7.41 → 5.60，切片残差 4.9 vs 仅阻抗 5.8 vs 仅钻孔 11.6。
