...
本文给出本仓库三维岩石强度场 \(S(x,y,z)\) 的完整计算方案：从随钻参数与叠后地震出发，经可推导的正反演与克里金，再到外漂移克里金融合。所有符号与实现一一对应（见文末模块对照）。定量结论来自固定种子的共定位合成矿山基准（`seed=42`，12 口钻孔，网格 \(25\times 40\times 40\)，块段 \(50\times 80\times 40\,\mathrm{m}\)），图件在 [`images/`](images/) 中跟踪保存。
...
下图上一行同一 UCS 色标：真值、仅钻孔、仅波阻抗标定、融合。下一行是预测减去真值的差距热力图（红偏高、蓝偏低，三列共用色标）：仅钻孔看不见硬矿体；仅波阻抗轮廓对了但蚀变带估计偏高；融合残差最浅，矿体峰值不再被压矮。
