...
仅用钻孔把 UCS **插值**到三维，即使缩到 \(50\times 80\,\mathrm{m}\) 的爆破块、12 口孔按梅花均匀布置，相邻孔距仍约 15–25 m。克里金在孔旁忠实，孔间变光滑：硬矿体轮廓发糊，断裂和蚀变晕对不上。
...
- 12 口孔按爆破常用的**梅花 / 三角网格**近似均匀铺在块段上（邻行错半个孔距，不必严格等边）。1→N 实验的嵌套前缀按最远点排序，稀疏子集同样铺开，而不是挤在一角。孔内每个深度节点都有样本。
...
- 12 口孔按梅花铺在 \(50\times 80\,\mathrm{m}\) 平面上，相邻孔距约 15–25 m，胜任度场仍含有矿体/软弱带尺度的横向结构；
...
在本基准上简单平均 \(R^2=0.555\)，高于仅地震的 \(0.491\) 和仅钻孔的 \(0.460\)，但低于精度加权的 \(0.557\)。梅花布孔之后 \(S_M\) 不再大面积空白，简单平均不再明显砸结构；精度加权仍然略好，并多给出 \(\sigma_F\) 与 \(w_{\mathrm{MWD}}\)。
...
下图同一 UCS 色标：真值、仅钻孔、仅波阻抗标定、融合。三张预测图差得开：仅钻孔是梅花靶心插值、几乎看不见硬矿体；仅波阻抗能勾出矿体轮廓，但蚀变晕和峰值对不上；融合把阻抗的连续结构与孔旁力学锚点合在一起，硬斑比纯阻抗更接近真值，又不会把克里金的靶心伪影写满全场。
...
2. **在孔轨迹上钉住力学真值。** 蚀变、风化会改 UCS，却几乎不改 AI。沿孔剖面里，橙色“仅波阻抗”会偏离真值，绿色融合跟着 MWD 回去。12 口孔轨迹上：仅波阻抗 RMSE \(=9.8\,\mathrm{MPa}\)，融合 \(=2.6\,\mathrm{MPa}\)。
...
所以：钻孔**不是用来把全矿插值填满的**（那件事波阻抗更擅长），而是**标定器 + 孔旁锚点**。梅花布置让这些锚点均匀盖住块段，插值本身也有用（仅钻孔 \(R^2=0.46\)），但体积误差仍高于融合，平面图上硬矿体峰值明显偏弱。
...
4. 与同一份 \(\mathrm{UCS}_{\mathrm{true}}\) 比 \(R^2\)、RMSE；并单独统计距最近孔超过 \(0.4\times\) 中位孔距（本基准约 \(9.8\,\mathrm{m}\)）的远场 RMSE，以及孔轨迹上的 RMSE。
...
| ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| 1 | 13.45 | 0.099 | 15.84 | 14.83 | −0.096 | 4.64 | 0.00 |
| 2 | 12.10 | 0.270 | 14.57 | 12.98 | 0.160 | 7.42 | 2.08 |
| 4 | 11.52 | 0.339 | 9.70 | 9.64 | 0.537 | 8.30 | 2.61 |
| 6 | 11.51 | 0.340 | 10.99 | 10.65 | 0.435 | 9.16 | 2.76 |
| 8 | 10.88 | 0.410 | 9.70 | 9.31 | 0.568 | 9.91 | 2.58 |
| 12 | 10.41 | 0.460 | 10.11 | 9.43 | 0.557 | 9.83 | 2.61 |

（完整表见 [`images/well_count_metrics.csv`](images/well_count_metrics.csv)。远场阈值为 \(0.4\times\) 中位孔距。）

读表：

1. **一口井时融合不会盲目信阻抗。** \(n=1\) 的标定样本少，仅波阻抗 RMSE 15.8，比纯插值 13.4 还差；融合 14.8，介于两者之间。
2. **孔数够做标定时，体积上融合优于两路单独。** \(n=12\) 时 RMSE 9.43，低于仅波阻抗 10.11 和仅钻孔 10.41；平面图上三张图差得开，不再塌成“全是阻抗”。
3. **孔上融合 ≈ MWD，远好于仅波阻抗。** 12 口孔轨迹上 2.61 vs 9.83。蚀变/硬矿体剖面上橙色阻抗漂峰值，绿色融合钉回真值。
4. **加孔主要改善插值和标定，结构仍来自阻抗。** 仅钻孔 \(R^2\) 从 0.10 升到 0.46；融合场的硬矿体轮廓在 \(n=3\)–\(4\) 已经出现。
...
| --- | ---: | ---: | ---: |
| 仅 MWD 克里金插值 | 0.460 | 10.41 | 7.07 |
| 仅波阻抗标定 | 0.491 | 10.11 | 6.68 |
| 简单加权 \(w=1/2\) | 0.555 | 9.46 | 6.60 |
| **不确定度加权融合（本方案）** | **0.557** | **9.43** | **6.55** |

补充：波阻抗反演本身 \(R^2(\mathrm{AI})=0.839\)。孔轨迹上融合 RMSE \(=2.61\)，仅波阻抗 \(=9.83\)。

- 相对**仅钻孔**，融合把体积 RMSE 从 10.41 降到 9.43，平面图上硬矿体从“看不见”变成有轮廓；
- 相对**仅波阻抗**，体积 RMSE 10.11 → 9.43，孔旁把力学残差钉回去（见沿孔剖面）；
- 相对**简单平均**，精度加权略好（9.46 → 9.43），并给出 \(\sigma_F\) 和 \(w_{\mathrm{MWD}}\)。
