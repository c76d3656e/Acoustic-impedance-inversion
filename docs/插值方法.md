...
合成块段平面为 \(50\times 80\,\mathrm{m}\)、垂向 \(40\,\mathrm{m}\)（约十个台阶）。12 口梅花孔布置在中心 \(20\times 50\,\mathrm{m}\) 工作面内部；可视化默认只显示该区间，并可在全块上平移。图 1 给出梅花布孔与工作面窗口。

![图 1　梅花布孔（左）与默认 20×50 m 工作面窗口（右）。12 口孔全部落在 20×50 m 工作面内；全块仍为 50×80 m 以便平移。](images/hole_layout.png)
...
沿孔剖面里，钻孔插值与真值几乎重合（硬数据），这不能推广到孔间：工作面体积 RMSE 仍为 \(8.65\,\mathrm{MPa}\)，\(R^2=0.58\)（12 口孔，\(20\times 50\,\mathrm{m}\)）。
...
体积 RMSE \(8.11\,\mathrm{MPa}\)，略优于钻孔插值；孔轨迹上 RMSE \(7.04\,\mathrm{MPa}\)，明显差于硬数据。把 \(S_Z\) 当作另一张可以和 \(S_M\) 对半平均的强度图，会在孔上冲掉真值。
...
假设不成立：两路共用同一批标定井。简单平均的工作面体积 RMSE 为 \(7.41\,\mathrm{MPa}\)，KED 为 \(5.60\,\mathrm{MPa}\)。融合插值不是“两张图的加权”，而是“地震为漂移、井为硬数据”的一次克里金。
...
定量（\(20\times 50\,\mathrm{m}\) 工作面、12 口孔、`seed=42`）：

| 方法 | \(R^2\) | RMSE (MPa) | MAE (MPa) |
| --- | --- | --- | --- |
| 钻孔插值 | 0.575 | 8.65 | 5.36 |
| 波阻抗插值 | 0.626 | 8.11 | 4.52 |
| 简单平均 | 0.688 | 7.41 | 4.69 |
| 融合插值 | 0.822 | 5.60 | 3.45 |

孔轨迹上融合 RMSE \(=2.49\,\mathrm{MPa}\)（与钻孔插值相同），波阻抗插值 \(=7.04\,\mathrm{MPa}\)：硬数据被钉住。图 7 沿蚀变晕附近钻孔与硬矿体附近钻孔画出四条曲线，波阻抗插值偏离真值，融合插值与钻孔插值重合。
...
随钻孔数从 1 增至 12（图 9）：一口井时融合走 Doyen，不会把过拟合的 GP 铺满全块；两口井起 KED，体积误差低于两路单独。加孔主要改善钻孔插值与标定，硬矿体轮廓在 \(n=2\)–\(4\) 已由波阻抗给出。
...
克里金代价随孔点数与网格体积上升。合成全块保持 \(50\times 80\times 40\,\mathrm{m}\)、网格 \(25\times 40\times 40\)，12 口梅花孔铺在中心 \(20\times 50\,\mathrm{m}\) 工作面内。前端与出版图默认只显示该工作面，并提供 \(X\)、\(Y\) 原点滑条在全块上平移窗口。这是可视化截取，不是重新插值：三维场仍在全块上一次算完。
