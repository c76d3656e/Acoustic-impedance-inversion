...
合成块段平面为 \(50\times 80\,\mathrm{m}\)、垂向 \(40\,\mathrm{m}\)（约十个台阶）。可视化默认只截取 \(20\times 50\,\mathrm{m}\) 工作面，并可在全块上平移；孔数保持 12 口不变。图 1 给出梅花布孔与工作面窗口。

![图 1　梅花布孔（左）与默认 20×50 m 工作面窗口（右）。全块仍为 50×80 m，钻孔数量不变。](images/hole_layout.png)
...
沿孔剖面里，钻孔插值与真值几乎重合（硬数据），这不能推广到孔间：体积 RMSE 仍为 \(9.49\,\mathrm{MPa}\)，\(R^2=0.55\)（12 口孔，全块）。
...
闭式解 \(Z=\exp(m)\)。低频背景来自重平滑趋势（现场则来自井曲线沿层位插值）。本基准 \(\mathrm{AI}\) 反演相对真值 \(R^2=0.839\)。
...
体积 RMSE \(9.01\,\mathrm{MPa}\)，略优于钻孔插值；孔轨迹上 RMSE \(7.94\,\mathrm{MPa}\)，明显差于硬数据。把 \(S_Z\) 当作另一张可以和 \(S_M\) 对半平均的强度图，会在孔上冲掉真值。
...
假设不成立：两路共用同一批标定井。简单平均的体积 RMSE 为 \(7.55\,\mathrm{MPa}\)，KED 为 \(6.84\,\mathrm{MPa}\)。融合插值不是“两张图的加权”，而是“地震为漂移、井为硬数据”的一次克里金。
...
| --- | --- | --- | --- |
| 钻孔插值 | 0.552 | 9.49 | 6.43 |
| 波阻抗插值 | 0.595 | 9.01 | 5.68 |
| 简单平均 | 0.716 | 7.55 | 5.39 |
| 融合插值 | 0.767 | 6.84 | 4.64 |

孔轨迹上融合 RMSE \(=2.56\,\mathrm{MPa}\)（与钻孔插值相同），波阻抗插值 \(=7.94\,\mathrm{MPa}\)：硬数据被钉住。图 7 沿蚀变晕附近钻孔与硬矿体附近钻孔画出四条曲线，波阻抗插值偏离真值，融合插值与钻孔插值重合。
...
克里金代价随孔点数与网格体积上升。合成全块保持 \(50\times 80\times 40\,\mathrm{m}\)、网格 \(25\times 40\times 40\)，前端默认只显示中心 \(20\times 50\,\mathrm{m}\) 工作面，并提供 \(X\)、\(Y\) 原点滑条在全块上平移窗口。孔数不变，窗口外的孔在当前视图中隐藏。这是可视化截取，不是重新插值：三维场仍在全块上一次算完。
